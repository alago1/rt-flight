...
function createRipple(markerIcon) {
  ...
  const { x, y } = Array.from(
    CSSStyleValue.parse("transform", markerIcon.style.transform)
...
async function load_markers(bbox_filter = null) {
  ...
  const markers_url =
    "/api/markers/" + (bbox_filter !== null ? `?${queryParams}` : "");
...
async function render_markers({ skip_on_no_new = false }) {
  ...
  if (
    skip_on_no_new &&
    markers.features.every((f) => knownIds.has(f.properties.id))
  ) {
    return;
  }
  ...
  clusters.forEach((cluster, i) =>
    cluster.forEach((idx) => (cluster_labels[idx] = i))
  );

  // add it as a feature so that it can be used in the popup
  markers.features.forEach(
    (feature, i) => (feature.properties.cluster = cluster_labels[i])
  );
  ...
  const heatmap_data = markers.features.map((feature) => ({
    ...
    radius: Math.max(
      Math.min(
        (feature.properties.radius / metersPerPixel) * scale,
        max_radius
      ),
      min_radius
    ),
  ...
  if (markerLayer !== null) {
    ...
    const new_markers_ids = new Set(
      markers.features.map((f) => f.properties.id)
    );
    const removable_layer_keys = Object.keys(markerLayer._layers).filter(
      (k) => !new_markers_ids.has(markerLayer._layers[k].feature.id)
    );
    removable_layer_keys.forEach((key) =>
      markerLayer._layers[key].removeFrom(map)
    );
  }

  // find markers that are not in the layer (new markers) and add them to the map
  const existing_markers_ids = new Set(
    Object.keys(markerLayer?._layers ?? {}).map(
      (key) => markerLayer._layers[key].feature.id
    )
  );
  const new_markers = markers.features.filter(
    (f) => !existing_markers_ids.has(f.properties.id)
  );

  markerLayer = L.geoJSON(new_markers).bindPopup(markerPopup).addTo(map);

  const dom_icons = Object.keys(markerLayer._layers)
    .map((key) => ({
  ...
  dom_icons.forEach(({ id, icon, cluster }) => {
    ...
    icon.setAttribute(
      "style",
      `${prev_style}; filter: ${cluster_colors[cluster]}`
    );
  ...
  if (heatmap_data.length > 0) heatmapLayer.setData({ data: heatmap_data });
}
